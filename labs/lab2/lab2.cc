...
void count(float* x, int n, float t, int &nb, int rank, int nprocs, int xory)
{
	...
	ierr = MPI_Scatter(test, size, MPI_FLOAT, localData, size, MPI_FLOAT, 0, MPI_COMM_WORLD);
	for (i=0; i < size; i++){
		if (localData[i] < t){
			localCounter++;
		}
	}
	ierr = MPI_Reduce(&localCounter, &nb, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
...
int main(int argc, char* argv[]){
	...
	int n = 32768, nbounds = n+2, nbx = 0, nby = 0, i, j;
	...
	if (rank == 0) time5 = MPI_Wtime();
		count(x, n, t, nbx, rank, nprocs, 1);
	if (rank == 0) time6 = MPI_Wtime();
		count(y, n, t, nby, rank, nprocs, 2);
	if (rank == 0) time7 = MPI_Wtime();
